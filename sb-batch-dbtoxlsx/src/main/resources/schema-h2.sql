...
CREATE TABLE IF NOT EXISTS cron_tbl (
  	cid NUMBER(10,0) NOT NULL AUTO_INCREMENT,
 	cron_expression varchar(45) DEFAULT NOT NULL,
 	cron_desc varchar(255) DEFAULT NULL
);

INSERT INTO cron_tbl (cid, cron_expression, cron_desc) VALUES 
(1, '0 */1 * * * ?', 'At second :00, every minute starting at minute :00, of every hour'),
(2, '0 36 01 * * ?', 'At 19:53:00pm, every day starting on the 1st, every month');
...
CREATE TABLE IF NOT EXISTS `user_details` (
  ...
  `status` NUMBER(10,0) DEFAULT NULL,
  PRIMARY KEY (`user_id`)
);
